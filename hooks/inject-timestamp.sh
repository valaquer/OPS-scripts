...
# Active room injection — fetch the teammate's current Facade rooms
if [ -n "$TEAMMATE" ]; then
	ACTIVE_JSON=$(curl -s --connect-timeout 1 "http://localhost:51730/api/rooms/active-room?name=$TEAMMATE" 2>/dev/null)
