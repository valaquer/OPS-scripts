...
preflight_check "/Users/deepak-macmini/honeybloom" "Honeybloom workspace" || skip_workspace=1
preflight_check "/Users/deepak-macmini/.claude" "Claude Code config" || skip_claude=1
...
deploy_item "honeybloom workspace (this will take a while)" \
    "$SRC/workspace" \
    "/Users/deepak-macmini/honeybloom" \
    "$skip_workspace"

# 2. Claude Code config directory
deploy_item "~/.claude/ config directory" \
    "$SRC/claude-config" \
    "/Users/deepak-macmini/.claude" \
    "$skip_claude"

# 3. Claude Code global config
if [[ -f "$SRC/claude-global/.claude.json" ]]; then
    if [[ -f "/Users/deepak-macmini/.claude.json" && ! -f "/Users/deepak-macmini/.claude.json.pre-transfer-backup" ]]; then
        mv "/Users/deepak-macmini/.claude.json" "/Users/deepak-macmini/.claude.json.pre-transfer-backup"
        echo "  Backed up existing ~/.claude.json"
    fi
    echo "  Deploying: ~/.claude.json (MCP servers)"
    cp "$SRC/claude-global/.claude.json" "/Users/deepak-macmini/.claude.json"
fi
...
deploy_item "Kitty config (kitty.conf + session files)" \
    "$SRC/kitty-config" \
    "/Users/deepak-macmini/.config/kitty"

# 6. LaunchAgent plists
if [[ -d "$SRC/launchagents" ]]; then
    echo "  Deploying: LaunchAgent plists"
    mkdir -p "/Users/deepak-macmini/Library/LaunchAgents"
    for plist in "$SRC/launchagents"/com.honeybloom.*.plist; do
        if [[ -f "$plist" ]]; then
            name="$(basename "$plist")"
            cp "$plist" "/Users/deepak-macmini/Library/LaunchAgents/"
            echo "    $name"
        fi
    done
    ...
    local uid
    uid="$(id -u)"
    for plist in /Users/deepak-macmini/Library/LaunchAgents/com.honeybloom.*.plist; do
        if [[ -f "$plist" ]]; then
            name="$(basename "$plist")"
...
if [[ -f "/Users/deepak-macmini/honeybloom/samara/reconcile-skills.py" ]]; then
    python3 /Users/deepak-macmini/honeybloom/samara/reconcile-skills.py 2>&1 && echo "  Symlinks rebuilt." || echo "  WARNING: reconcile-skills.py failed"
else
